...
gulp.task('watch', function () {
    ...
    gulp.watch(defaultAssets.server.allJS, ['eslint']).on('change', plugins.livereload
        .changed);
    gulp.watch(defaultAssets.client.js, ['eslint']).on('change', plugins.livereload
        .changed);
    ...
    if (process.env.NODE_ENV === 'production') {
        ...
        gulp.watch(defaultAssets.client.views, ['templatecache']).on('change',
            plugins.livereload.changed);
    } else {
...
gulp.task('watch:server:run-tests', function () {
    ...
    gulp.watch([testAssets.tests.server, defaultAssets.server.allJS], [
        'test:server'
...
gulp.task('less', function () {
    return gulp.src('modules/*/client/less/app.less')
        .pipe(plugins.less())
        .pipe(plugins.autoprefixer())
        .pipe(plugins.rename(function (file) {
...
gulp.task('mocha', function (done) {
    ...
    mongoose.connect(function () {
        mongoose.loadModels();
        // Run the tests
        var gulp1 = gulp.src(testSuites);
        gulp1.pipe(plugins.mocha({
            reporter: 'spec',
            timeout: 100000
        }))
...
gulp.task('test', function (done) {
    runSequence('env:test', 'test:server', 'karma', 'nodemon', 'protractor',
        done);
});

gulp.task('test:server', function (done) {
    runSequence('env:test', ['copyLocalEnvConfig', 'makeUploadsDir', 'dropdb'],
        'lint', 'mocha', done);
});

gulp.task('dev.test:server', function (done) {
...
gulp.task('default', function (done) {
    runSequence('env:dev', ['copyLocalEnvConfig', 'makeUploadsDir'], 'lint', 'moveAngulrLibs', [
        'nodemon', 'watch'
    ], done);
});
...
gulp.task('debug', function (done) {
    runSequence('env:dev', ['copyLocalEnvConfig', 'makeUploadsDir'], 'lint', 'moveAngulrLibs', [
        'node-inspector', 'nodemon-debug', 'watch'
    ], done);
});

// Run the project in production mode
gulp.task('prod', function (done) {
    runSequence(['copyLocalEnvConfig', 'makeUploadsDir', 'templatecache'],
        'build', 'env:prod', 'lint', 'moveAngulrLibs', ['nodemon', 'watch'], done);
});

gulp.task('moveAngulrLibs', function () {
    return gulp.src(defaultAssets.client.moveAngulrLibs)
