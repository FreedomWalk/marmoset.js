...
let initGlobalConfigFiles = function (config, assets) {
  ...
  // Setting Globbed schedules files
  config.files.server.schedules = getGlobbedPaths(assets.server.schedules);

  config.files.server.receivers = getGlobbedPaths(assets.server.receivers);
